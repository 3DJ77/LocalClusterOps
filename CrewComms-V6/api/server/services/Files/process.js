...
const shouldUseMessageTextContext = ({ metadata, file }) => {
  if (!metadata.message_file || metadata.tool_resource || file.mimetype.startsWith('image')) {
    return false;
  }

  const endpoint = normalizeUploadEndpoint(metadata.endpoint);
  const endpointType = normalizeUploadEndpoint(metadata.endpointType);

  return endpointType === EModelEndpoint.custom || autoContextMessageEndpoints.has(endpoint);
};
...
const processAgentFileUpload = async ({ req, res, metadata }) => {
  ...
  const effectiveToolResource = shouldUseMessageTextContext({ metadata, file })
    ? EToolResources.context
    : tool_resource;

  let messageAttachment = !!metadata.message_file;

  if (agent_id && !effectiveToolResource && !messageAttachment) {
