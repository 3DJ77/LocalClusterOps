...
describe('processAgentFileUpload', () => {
  ...
  describe('message attachment text context', () => {
    test('stores Task Runtime text message attachments as prompt-readable text context by default', async () => {
      ...
      req.file.originalname = 'note.txt';
      ...
      expect(parseText).toHaveBeenCalledWith({ req, file: req.file, file_id: 'file-uuid-123' });
      expect(createFile).toHaveBeenCalledWith(
        expect.objectContaining({
          file_id: 'file-uuid-123',
          filename: 'note.txt',
          source: FileSources.text,
          text: 'crew upload secret',
        }),
        true,
      );
    ...
    test('leaves non-custom message attachments on the normal storage path', async () => {
      const { parseText } = require('@librechat/api');
