...
const LOCAL_AUTH_ENABLED = process.env.REACT_APP_LOCAL_AUTH !== '0';
...
const AuthContextProvider = ({
  ...
  const [token, setToken] = useState<string | undefined>(
    LOCAL_AUTH_ENABLED ? LOCAL_TOKEN : undefined,
  );
  const [error, setError] = useState<string | undefined>(undefined);
  const [isAuthenticated, setIsAuthenticated] = useState<boolean>(LOCAL_AUTH_ENABLED);
  ...
  const userQuery = useGetUserQuery({ enabled: !!(token ?? '') && !LOCAL_AUTH_ENABLED });
